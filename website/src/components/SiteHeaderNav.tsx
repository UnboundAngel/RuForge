  ...
  return (
    <NavigationMenuLink asChild>
      <a
        ...
        style={{ height: FEATURED_CARD_HEIGHT }}
      >
        ...
        {item.variant === 'landscape' && (
          <div className="relative z-10 mt-auto flex justify-end px-3.5 pb-3.5 pt-1">
            <div
              className="pointer-events-none h-10 w-10 shrink-0 rounded-lg border border-[#edd79c]/12 bg-[#edd79c]/5"
              aria-hidden
            />
          </div>
        )}
...
const MegaPanel = memo(function MegaPanel({ sectionId }: { sectionId: NavSectionId }) {
  ...
  const linkColumns = (
    <ul
      className={cn(
        ...
        twoCols ? 'grid-cols-2 gap-x-8' : 'grid-cols-1',
      )}
    >
  ...
  const featuredAside =
    config.layout === 'links-featured-row' ? (
      <div className="rf-mega-menu-featured rf-scrollbar flex max-w-full shrink-0 items-stretch gap-2.5 pl-1">
      ...
      <div className="rf-mega-menu-featured rf-scrollbar flex max-w-full shrink-0 flex-col justify-start gap-2.5 pl-1">
        {config.featured.map((item) => (
          <FeaturedVisualCard
            key={item.slug}
            item={item}
      ...
      <div className="rf-mega-menu-featured max-w-full shrink-0 pl-1">
        <FeaturedVisualCard
  ...
  return (
    <div
      className={cn(
        'rf-mega-menu grid shrink-0 items-stretch gap-x-10 px-7 py-6',
        config.layout === 'links-icons'
          ? 'grid-cols-[minmax(0,1.55fr)_minmax(10.5rem,13.25rem)]'
