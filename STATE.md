...
Status: 0.2.1 live on GitHub, updater.json, and ruforge.app. LAN companion remains dev-gated only in tree.

## Now

0.2.1 shipped: canonical Rust library backend, updater island UX, download hardening, music playlist cover path. Fresh 0.2.2 cycle open. Companion LAN server in tree behind `showDebuggingSettings` only.
