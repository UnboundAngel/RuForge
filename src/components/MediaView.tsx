      ...
      <div className="absolute inset-0 glass rounded-2xl overflow-hidden shadow-2xl transition-all border border-white/10 z-10">
        <div className="absolute inset-0 bg-black/60 flex items-center justify-center overflow-hidden">
          ...
          <div className="absolute top-3 left-3 px-2 py-1 bg-[color:var(--accent)] rounded-full flex items-center gap-1.5 shadow-2xl z-20">
            ...
            <span className="text-[9px] font-black text-stone-950 uppercase tracking-widest">{playlist.itemCount} Items</span>
          </div>
...
const VideoCard = ({ 
  ...
  file: MediaFile, 
  index: number,
  activeMenu: { path: string, x: number, y: number } | null,
  setActiveMenu: (menu: { path: string, x: number, y: number } | null) => void,
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    e.preventDefault();
    setActiveMenu({ path: file.path, x: e.clientX, y: e.clientY });
  };
        ...
        <div className="relative self-start mt-0.5">
          <button 
            onClick={(e) => {
              ...
              setActiveMenu({ path: file.path, x: rect.right, y: rect.bottom });
            }} 
...
export const MediaView = ({
  ...
  const [activeMenu, setActiveMenu] = useState<{ path: string, x: number, y: number } | null>(null);
  const [extracting, setExtracting] = useState<string | null>(null);

  const loadFiles = async (opts?: { showLoading?: boolean; skipPosterBackfill?: boolean }) => {
    ...
    try {
      ...
      const unique = combined.filter((entry, index, self) =>
        index === self.findIndex((t) => t.path === entry.path)
      );
      
      setEntries(unique);
      
      if (!skipPosterBackfill) {
        const mediaFiles = unique.flatMap(e => e.kind === 'media' ? [e as MediaFile] : (e as PlaylistCollection).items);
        const need = filesMissingPoster(mediaFiles);
        if (need.length > 0) backfillList = need;
      }
    } catch (e) {
      console.error(e);
      onNotify("Failed to load video library.");
    } finally {
    ...
    if (backfillList) {
      void (async () => {
        ...
        await loadFiles({ showLoading: false, skipPosterBackfill: true });
      })();
    }
  };
  ...
  const handleDelete = async (file: MediaFile) => {
    if (confirm(`Are you sure you want to delete "${file.name}"?`)) {
      try {
        ...
        onNotify("Video deleted successfully.");
        loadFiles();
      } catch (e) {
        console.error(e);
        onNotify("Failed to delete video.");
      }
    }
    setActiveMenu(null);
  };
  ...
  const handleExtract = async (file: MediaFile) => {
    setExtracting(file.path);
    try {
      await invoke("extract_frames", { videoPath: file.path });
      onNotify("Previews generated successfully.");
      await loadFiles({ showLoading: false, skipPosterBackfill: true });
    } catch (e) {
      console.error(e);
      onNotify("Failed to generate previews.");
    } finally {
      setExtracting(null);
      setActiveMenu(null);
    }
  ...
  useEffect(() => {
    loadFiles();
  }, [outputDir, internalDir, saveToInternal]);
            ...
            {Object.entries(groupedEntries).map(([dateLabel, groupEntries]) => (
              <section key={dateLabel} className="space-y-8">
                <div className="flex items-center space-x-6">
                  ...
                  <span className="text-[9px] font-black text-stone-600 uppercase tracking-widest">{groupEntries.length} {groupEntries.length === 1 ? 'Source' : 'Sources'}</span>
                </div>
                  ...
                  {groupEntries.map((entry, i) => {
                    ...
                    return (
                      <VideoCard 
                        ...
                        onPlay={onPlay}
                        extracting={extracting === file.path}
                      />
                    );
                  })}
                </div>
            ...
            {(() => {
              ...
              return (
                <div className="p-1.5">
                  ...
                  <button 
                    onClick={() => { onPlay(file); setActiveMenu(null); }}
