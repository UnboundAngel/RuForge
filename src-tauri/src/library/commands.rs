...
/// result. The frontend calls this once per refresh instead of invoking
/// `scan_gallery` per root and deduping client-side.
#[tauri::command]
pub async fn get_library_snapshot(
    ...
    Ok(LibrarySnapshot { version, ready, entries })
}
